...
void multiTouchInfoInit()
{
	for(int i=0;i<12;i++)
	{
		for(int x=0;x<28;x++)
		{
			multiTouch[i][x]=false;
		}
	}
}
...
void multiTouchInfoPrint()
{
	...
	for(int i=0;i<12;i++)
	{
		for(int x=0;x<28;x++)
		{
			if(multiTouch[i][x]==true)
			{
			    serprintln("   Board: "+String(i)+"   Position: "+String(x));
			    triggerd=true;
